...
若 GitHub 顯示 `README.md` 有衝突，建議在命令列執行：
...
# 編輯 README.md，移除 <<<<<<< ======= >>>>>>>
git add README.md
git rebase --continue  # 或 git commit（若是 merge）
```

本分支目前的 README 已整併以下重點：
- 本機執行與 Docker 執行方式。
- `python -m compileall app.py src` 的執行目錄。
- GitHub Actions 在 Actions 分頁查看執行結果的方法。
